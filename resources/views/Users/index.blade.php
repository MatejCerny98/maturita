    ...
    <table class="table table-bordered">
        ...
        @foreach($users as $user)
            <tr>
                ...
                <td>
                    <form action="{{route('group.add', ['userid' => $user->id])}}" method="post">
                        @csrf
                        <select name="group">
                            @foreach($groups as $group)
                                <option value="{{$group->id}}" @if($user->group && $group->id == $user->group->id) selected @endif>{{$group->nazov}}</option>
                            @endforeach
                        </select>
                        <button type="submit">Pridať do skupiny</button>
                    </form>
                </td>
